...
//    array = @[@4,@5];
//    
//    NSLog(@"%p,%p,%p",array,self.array1,self.array2);
//    NSLog(@"%@,%@,%@",array,self.array1,self.array2);
...
- (void)testmuArray
{
    ...
    NSLog(@"%p,%p",self.array,mutableArray);
...
//    NSLog(@"%p,%p,%@,%@",array,self.mutableArray,array,self.mutableArray);
//    
//    [self.mutableArray removeObjectAtIndex:0];
    ...
    self.str2 = str1;
    str1 = @"2";
    NSLog(@"%p,%p",str1,self.str2);
    NSLog(@"%@,%@",str1,self.str2);
...
- (void)testArray
{
    ...
    NSLog(@"%@,%@,%@",mArray1,mArrayCopy1,mArrayMCopy1);
}
- (void)testString
{
    ...
    NSLog(@"%p,%p,%p,%p,%p,%p,%p",string1,stringCopy1,stringMuCopy1,mustring,stringCopy,mstringCopy,stringMuCopy);
...
//    为了防止外界修改name,就用copy,因为copy是生成了一份新的内存空间，，所以外界是不能修改值。
    ...
    NSLog(@"p.name = %@",model.name);
    NSLog(@"p.StrongName = %@",model.strongName);
    
    NSLog(@"name:%p,p.name:%p,p.StrongName:%p",name,model.name,model.strongName);
}
