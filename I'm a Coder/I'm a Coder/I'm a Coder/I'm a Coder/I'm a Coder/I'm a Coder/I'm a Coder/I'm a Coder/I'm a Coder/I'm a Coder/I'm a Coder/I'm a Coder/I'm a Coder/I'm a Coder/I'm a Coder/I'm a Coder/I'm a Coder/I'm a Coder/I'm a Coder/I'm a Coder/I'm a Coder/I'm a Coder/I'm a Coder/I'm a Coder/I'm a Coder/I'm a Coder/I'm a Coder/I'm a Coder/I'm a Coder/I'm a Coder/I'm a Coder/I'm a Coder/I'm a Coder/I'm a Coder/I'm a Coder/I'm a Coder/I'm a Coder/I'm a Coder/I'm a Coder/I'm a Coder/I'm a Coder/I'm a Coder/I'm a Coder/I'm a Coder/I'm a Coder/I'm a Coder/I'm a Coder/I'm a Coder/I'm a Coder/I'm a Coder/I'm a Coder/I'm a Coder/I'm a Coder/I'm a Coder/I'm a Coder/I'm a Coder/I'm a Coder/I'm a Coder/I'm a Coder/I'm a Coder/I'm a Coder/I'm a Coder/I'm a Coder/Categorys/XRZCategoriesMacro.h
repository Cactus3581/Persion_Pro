...
#ifndef XRZCategoriesMacro_h
#define XRZCategoriesMacro_h

#ifndef XRZNotificationCenter
#define XRZNotificationCenter [NSNotificationCenter defaultCenter]
#endif

#ifndef XRZUserDefaults
#define XRZUserDefaults [NSUserDefaults standardUserDefaults]
#endif
...
#ifndef XRZ_CLAMP // 返回中间值
#define XRZ_CLAMP(_x_, _low_, _high_)  (((_x_) > (_high_)) ? (_high_) : (((_x_) < (_low_)) ? (_low_) : (_x_)))
#endif

#ifndef XRZ_SWAP // 值交换
#define XRZ_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif
...
/**
 ...
 使用：
 XRZSYNTH_DUMMY_CLASS(NSString_XRZAdd)
 */
#ifndef XRZSYNTH_DUMMY_CLASS
#define XRZSYNTH_DUMMY_CLASS(_name_) \
@interface XRZSYNTH_DUMMY_CLASS_ ## _name_ : NSObject @end \
@implementation XRZSYNTH_DUMMY_CLASS_ ## _name_ @end
#endif

/**
 自定义NSLog
 使用 : XRZLog("你好%@",@"wazrx")，第一个"前面无需添加@,添加了也无所谓
...
#ifndef XRZEdgeInsetsOne
#define XRZEdgeInsetsOne(_s_) UIEdgeInsetsMake(_s_, _s_, _s_, _s_)
#endif

#ifndef XRZEdgeInsetsTwo
#define XRZEdgeInsetsTwo(_a_, _b_) UIEdgeInsetsMake(_a_, _b_, _a_, _b_)
#endif
