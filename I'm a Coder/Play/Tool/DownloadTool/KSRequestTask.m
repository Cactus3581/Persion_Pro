...
@interface KSRequestTask ()<NSURLConnectionDataDelegate, NSURLSessionDataDelegate>
...
@implementation KSRequestTask

- (instancetype)init {
    if (self = [super init]) {
        [KSFileHandle createTempFile];
    }
    return self;
}
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    if (self.cancel) return;
    [KSFileHandle writeTempFileData:data];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    if (self.cancel) {
        ...
        if (error) {
            ...
            //可以缓存则保存文件
            if (self.cache) {
                [KSFileHandle cacheTempFileWithFileName:[NSString fileNameWithURL:self.requestURL]];
            }
