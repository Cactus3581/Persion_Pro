...
@interface KSResourceLoader ()

@property (nonatomic, strong) NSMutableArray * requestList;
@property (nonatomic, strong) KSRequestTask * requestTask;

@end

@implementation KSResourceLoader
...
- (void)newTaskWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest cache:(BOOL)cache {
    ...
    self.requestTask = [[KSRequestTask alloc]init];
...
- (BOOL)finishLoadingWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    [loadingRequest.dataRequest respondWithData:[KSFileHandle readTempFileDataWithOffset:requestedOffset - self.requestTask.requestOffset length:respondLength]];
    //如果完全响应了所需要的数据，则完成
