...
@interface KSQRCodeScanTool()<AVCaptureMetadataOutputObjectsDelegate> {
...
//扫码结果
@property (nonatomic, strong) NSMutableArray<KSQRCodeScanResult*> *arrayResult;
 ...
 *  扫码结果返回
 */
@property(nonatomic,copy)void (^blockScanResult)(NSArray<KSQRCodeScanResult*> *array);
@end

@implementation KSQRCodeScanTool
- (void)setNeedCaptureImage:(BOOL)isNeedCaputureImg {
    _isNeedCaputureImage = isNeedCaputureImg;
}

- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect success:(void(^)(NSArray<KSQRCodeScanResult*> *array))block {
    if (self = [super init]) {
...
- (void)initParaWithPreView:(UIView*)videoPreView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect success:(void(^)(NSArray<KSQRCodeScanResult*> *array))block {
...
- (void)captureImage {
    ...
    [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:stillImageConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
         ...
         if (imageDataSampleBuffer) {
             ...
             for (KSQRCodeScanResult* result in _arrayResult) {
    ...
    for(AVMetadataObject *current in metadataObjects) {
        if ([current isKindOfClass:[AVMetadataMachineReadableCodeObject class]] ) {
            ...
            if (scannedResult && ![scannedResult isEqualToString:@""]) {
                KSQRCodeScanResult *result = [KSQRCodeScanResult new];
...
#pragma mark --识别条码图片
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray<KSQRCodeScanResult*> *array))block; {
    if ([[[UIDevice currentDevice]systemVersion]floatValue] < 8.0 ) {
        if (block) {
            KSQRCodeScanResult *result = [[KSQRCodeScanResult alloc]init];
    ...
    NSMutableArray<KSQRCodeScanResult*> *mutableArray = [[NSMutableArray alloc]initWithCapacity:1];
    for (int index = 0; index < [features count]; index ++) {
        CIQRCodeFeature *feature = [features objectAtIndex:index];
        NSString *scannedResult = feature.messageString;
        NSLog(@"result:%@",scannedResult);
        KSQRCodeScanResult *item = [[KSQRCodeScanResult alloc]init];
