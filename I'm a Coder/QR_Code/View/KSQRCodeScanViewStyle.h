...
/**
 扫码区域动画效果
 */
typedef NS_ENUM(NSInteger,KScanViewAnimationStyle) {
    KScanViewAnimationStyle_LineMove,   //线条上下移动
    KScanViewAnimationStyle_NetGrid,//网格
    KScanViewAnimationStyle_LineStill,//线条停止在扫码区域中央
    KScanViewAnimationStyle_None    //无动画
};

/**
 扫码区域4个角位置类型
 */
typedef NS_ENUM(NSInteger, KScanViewPhotoframeAngleStyle) {
    KScanViewPhotoframeAngleStyle_Inner,//内嵌，一般不显示矩形框情况下
    KScanViewPhotoframeAngleStyle_Outer,//外嵌,包围在矩形框的4个角
    KScanViewPhotoframeAngleStyle_On   //在矩形框的4个角上，覆盖
};
...
/**
 @brief  扫码区域的4个角类型
 */
@property (nonatomic, assign) KScanViewPhotoframeAngleStyle photoframeAngleStyle;
...
/**
 @brief  扫码动画效果:线条或网格
 */
@property (nonatomic, assign) KScanViewAnimationStyle anmiationStyle;
